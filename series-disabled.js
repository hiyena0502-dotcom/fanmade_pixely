(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      #card-detail-series { display:none !important; }
      #card-detail-series + * { display:none !important; }
      #card-detail-series { visibility:hidden !important; }
    `;
  ...
  function cleanCopy() {
    ...
    const recentCopy = document.querySelector('#home-recent-card-copy');
    if (recentCopy && !recentCopy.textContent.trim()) recentCopy.textContent = '가챠에서 만난 추억을 컬렉션에 모아보세요.';
  ...
  const observer = new MutationObserver(() => keepSeriesUiHidden());
